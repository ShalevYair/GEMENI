...
**Input-token-window guards** (the model rejects requests over 1,048,576 input tokens; Hebrew runs ~1.5–2 chars/token): (a) proactive total-material caps per call — `fileBlocksFor(limit, files, totalCap, shrink)` shrinks the per-file limit so all files together stay within `CAP_CALIBRATION`/`CAP_MATERIALS`/`CAP_CHANGE_DOCS`; (b) `hardSplit` slices block pieces that have no natural boundaries (e.g. a DOCX whose whole body is one giant `<table>`) so no revise block exceeds ~4.8K chars; (c) reactive recovery — `callWithFallback` detects the token-limit error, disables the cache, and rebuilds the request with halved file caps (up to 3 shrink steps), and `patchOneSegment` bisects a revise segment that still doesn't fit, recursively.
