...
import { getRequirementsClarificationPrompt, getRequirementsChunkPrompt, REQUIREMENTS_CHECKLIST, INTERVIEW_QUESTIONS } from './requirements-prompt.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (agentId === 'requirements') injectRequirementsModal();
  ...
  empty.innerHTML = `
    ...
    <div class="suggestion-chips">
      ...
      ${agentId === 'requirements'
        ? `<button class="chip chip--generate" onclick="window.openRequirementsModal()">📋 צור SRS / שאלות הבהרה מחומרים גולמיים</button>`
        : ''}
      ${agentId === 'requirements'
...
function injectRequirementsModal() {
  ...
  const checklistHtml = REQUIREMENTS_CHECKLIST.map(item => {
    ...
    return `
      <label style="display:flex;align-items:center;gap:.45rem;cursor:pointer;padding:.25rem .35rem;border-radius:6px;transition:background .15s;font-size:.82rem;" onmouseenter="this.style.background='#f1f5f9'" onmouseleave="this.style.background=''">
        <input type="checkbox" id="rq-check-${escHtml(item.id)}" ${isChecked ? 'checked' : ''} style="accent-color:#059669;width:14px;height:14px;flex-shrink:0;">
        <span style="color:${item.defaultOn ? '#1e293b' : '#475569'}">${escHtml(item.label)}</span>
      </label>`;
  }).join('');
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:2rem;max-width:560px;width:100%;direction:rtl;box-shadow:0 20px 60px rgba(0,0,0,.25);font-family:Heebo,sans-serif;margin:auto;">
      ...
      <p style="margin:0 0 1.35rem;color:#6b7a99;font-size:.88rem;">העלה פרוטוקולי ישיבה, אימיילים, מסמכים גולמיים — וקבל SRS מובנה או שאלות הבהרה.</p>
      ...
      <div style="margin-bottom:1.1rem;">
        ...
        <label style="display:flex;align-items:flex-start;gap:.5rem;margin-bottom:.5rem;cursor:pointer;">
          ...
          <span><strong>מסמך SRS</strong> <span style="color:#6b7a99;font-size:.82rem;">— מפרט דרישות תוכנה מובנה עם Gap Analysis (3 חלקים)</span></span>
        </label>
        <label style="display:flex;align-items:flex-start;gap:.5rem;cursor:pointer;">
      ...
      <div id="rq-checklist-section" style="margin-bottom:1.35rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem;">
          ...
          <div style="display:flex;gap:.4rem;">
            <button onclick="window.rqSelectAll()" style="font-size:.75rem;padding:.2rem .55rem;border:1px solid #c8d0e0;border-radius:5px;background:#fff;cursor:pointer;">בחר הכל</button>
            <button onclick="window.rqSelectDefaults()" style="font-size:.75rem;padding:.2rem .55rem;border:1px solid #c8d0e0;border-radius:5px;background:#fff;cursor:pointer;">ברירת מחדל</button>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.1rem;border:1px solid #e2e8f0;border-radius:8px;padding:.5rem;">
          ${checklistHtml}
...
window.generateRequirements = async function () {
  ...
  if (mode === 'questions') {
    ...
    while (!done) {
      try {
        results.push(await callGeminiForArchitectSpec(prompt, rqModelIdx));
        done = true;
      } catch (err) {
        const quota = isQuotaExceeded(err.message);
        const busy  = /503|high demand|overload|temporarily/i.test(err.message);
        if ((quota || busy) && rqModelIdx < MODEL_CHAIN.length - 1) {
          rqModelIdx++;
          appendMessage('error', `⚠️ עובר למודל ${MODEL_CHAIN[rqModelIdx]}… 🔄`);
          await new Promise(r => setTimeout(r, 2000));
        } else {
          removeTyping(progressId);
          setLoading(false);
    ...
    const labels = lang === 'he'
      ? ['מבוא, בעלי עניין והנחות', 'דרישות פונקציונליות ו-Gap Analysis', 'סיכונים, דרישות נוספות וגלוסרי']
    ...
    for (let chunk = 1; chunk <= 3; chunk++) {
      ...
      while (!done) {
        try {
          ...
          if ((quota || busy) && rqModelIdx < MODEL_CHAIN.length - 1) {
            rqModelIdx++;
            appendMessage('error', `⚠️ עובר למודל ${MODEL_CHAIN[rqModelIdx]} (חלק ${chunk})… 🔄`);
            await new Promise(r => setTimeout(r, 2000));
          } else {
            removeTyping(progressId);
            setLoading(false);
  ...
  const header = isQ
    ...
    : `<!-- SRS — אוסף הדרישות | ${lang.toUpperCase()} | ${timestamp} | מקורות: ${fileNames} -->\n\n`;
  const combined = header + results.join('\n\n---\n\n');
  const filename = isQ
  ...
  appendMessage('assistant', isQ
    ? `✅ שאלות ההבהרה הורדו כ-\`${filename}\`\n\n**מקורות שנותחו (${requirementsFiles.length}):** ${fileNames}\n\nהשאלות מאורגנות לפי קטגוריות עם עדיפות (BLOCKER / IMPORTANT / NICE TO KNOW). לאחר קבלת תשובות — העלה שוב לייצור מסמך SRS.`
    : `✅ מסמך ה-SRS הורד כ-\`${filename}\`\n\n**שפה:** ${lang === 'he' ? 'עברית' : 'English'} · **מקורות (${requirementsFiles.length}):** ${fileNames}\n\n**חלקים שנוצרו:** ${results.length} מתוך 3\n\nלאחר אישור מסמך הדרישות, ניתן להעביר למלך האיפיונים לייצור FSD מפורט.`
  );
};

// ─── DOWNLOAD INTERVIEW QUESTIONS (static Word/Markdown) ────────────────────
window.downloadInterviewQuestions = function () {
  const timestamp = new Date().toISOString().slice(0, 10);

  // Build HTML for Word-compatible .doc
...
<p class="intro">מסמך זה מכיל שאלות גנריות לפגישת איסוף דרישות עם בעלי עניין. יש להתאים את השאלות לפרויקט הספציפי.<br>
תאריך: ${timestamp}</p>`;

  INTERVIEW_QUESTIONS.sections.forEach(section => {
    htmlContent += `<h2>${escHtml(section.title)}</h2><ol>`;
    section.questions.forEach(q => {
      htmlContent += `<li>${escHtml(q)}</li>`;
    });
    htmlContent += `</ol>`;
  });
  ...
  htmlContent += `
<div class="note">
  💡 <strong>טיפ:</strong> הורד מסמך זה ושלח אותו לבעלי העניין לפני הישיבה, כדי שיגיעו מוכנים עם תשובות ראשוניות.
  לאחר הישיבה — העלה את הפרוטוקול לסוכן "אוסף הדרישות" לקבלת מסמך SRS מובנה.
</div>
<div class="footer">נוצר על ידי אוסף הדרישות — אגם הסוכנים</div>
</body></html>`;

  try {
    // Try Word (.doc via HTML)
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `interview-questions-${timestamp}.doc`;
    a.click();
    URL.revokeObjectURL(url);
    appendMessage('assistant',
      ...
      `**המלצה:** שלח את המסמך לבעלי העניין לפני הפגישה. לאחר הישיבה — העלה את הפרוטוקול לכאן לקבלת SRS מובנה.`
    );
  } catch {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `interview-questions-${timestamp}.md`;
    a.click();
    URL.revokeObjectURL(url);
    appendMessage('assistant', `✅ שאלות הראיון הורדו כ-\`interview-questions-${timestamp}.md\``);
  }
};
