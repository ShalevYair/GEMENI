let chatHistory = [];
// Shared API key used across the whole app (see CLAUDE.md State Storage table),
// unlike the SDLC/NATURAL mind-map chats which keep a separate sessionStorage
// copy — reusing it here means the user isn't asked to re-enter a key they
// already set elsewhere in the app.
let geminiApiKey = localStorage.getItem('gemini_api_key') || '';
let chatWelcomeShown = false;
let typingIdCounter = 0;
let treeContextCache = null;
let chatFontPx = 14;
...
function onTreeLoaded() {
    treeContextCache = null; // invalidate cache when a new tree arrives
}

function buildTreeContext() {
    if (treeContextCache) return treeContextCache;
    if (typeof currentTree === 'undefined' || !currentTree) return 'מפת החשיבה עדיין לא נטענה.';
    let ctx = `סיכום היררכי של המסמך "${currentTree.name}":\n\n`;
    (currentTree.children || []).forEach(main => {
        ctx += `## נושא ראשי: ${main.name}\n`;
        if (main.desc) ctx += `תיאור: ${main.desc}\n`;
        (main.children || []).forEach(sub => {
            ctx += `  ### נושא משני: ${sub.name}\n`;
            if (sub.desc) ctx += `  תיאור: ${sub.desc}\n`;
            (sub.children || []).forEach(subsub => {
                ctx += `    #### תת נושא: ${subsub.name}\n`;
                if (subsub.desc) ctx += `    תיאור: ${subsub.desc}\n`;
            });
        });
        ctx += '\n';
    });
    treeContextCache = ctx;
    return ctx;
}
    ...
    if (geminiApiKey) {
        ...
        if (container.children.length === 0 && chatHistory.length > 0) {
            ...
            appendChatMessage('assistant',
                'שלום! אני כאן לעזור לך להבין את הסיכום שמוצג במפת החשיבה.\n' +
                'תוכל לשאול אותי על כל נושא, תת-נושא, או פרט שמופיע בתרשים.\n' +
                'במה אוכל לעזור?');
        }
...
async function callGeminiChat(userMessage) {
    ...
    const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${geminiApiKey}`,
        {
            ...
            body: JSON.stringify({
                system_instruction: {
                    parts: [{ text: 'אתה עוזר חכם שמסייע למשתמשים להבין סיכום מסמך היררכי דרך מפת חשיבה שנוצרה ממנו. ענה תמיד בעברית בלבד. הנה הסיכום המלא:\n\n' + buildTreeContext() }]
                },
                contents,
