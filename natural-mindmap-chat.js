...
function buildTreeContext() {
    ...
    if (typeof currentTree === 'undefined' || !currentTree) return 'מפת המחשבה עדיין לא נטענה.';
    let ctx = 'מפת המחשבה של קוד Natural שנותח, במבנה היררכי (JSON):\n\n';
    ctx += JSON.stringify(currentTree, null, 1);
    ...
    if (geminiApiKey) {
        ...
        if (container.children.length === 0 && chatHistory.length > 0) {
            ...
            appendChatMessage('assistant',
                'שלום! אני כאן לעזור לך להבין את הקוד שמוצג במפת המחשבה.\n' +
                'תוכל לשאול אותי על כל שלב, כלל עסקי, או צומת שמופיע בתרשים.\n' +
                'במה אוכל לעזור?');
        }
...
async function callGeminiChat(userMessage) {
    ...
    const res = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=${geminiApiKey}`,
        {
            ...
            body: JSON.stringify({
                system_instruction: {
                    parts: [{ text: 'אתה עוזר חכם שמסייע למשתמשים להבין קוד Natural (Software AG) דרך מפת מחשבה שנוצרה ממנו. ענה תמיד בעברית בלבד. הנה מבנה מפת המחשבה המלא:\n\n' + buildTreeContext() }]
                },
                contents,
